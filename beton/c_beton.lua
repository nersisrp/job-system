...
function betonAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 524 and vehicleJob == 14 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 14 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Beton Taşımacılığı Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		end
	end
end
...
function betonBlip()
	...
	exports.hud:sendBottomNotification(localPlayer, "Beton Taşımacılığı Şoförü", "Haritadaki sarı işarete giderek Beton Taşımacılığı şoförlüğü mesleğinize başlayabilirsiniz.")
end