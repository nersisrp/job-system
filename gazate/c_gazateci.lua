...
-- true, true bitiş
local gazeteRota = {
      { 1898.51953125, -1742.4033203125, 13.546875, false },
...
addCommandHandler("gazetebasla", gazeteBasla)
				...
				for _, marker in ipairs(gazeteCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!] #FFFFFFYeniden sürmek  için hazırlanıyoruz, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /gazetebitir yazınız.", 0, 0, 255, true)
							setTimer(
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitVehicle, true)
							toggleAllControls(false, true, false)
							outputChatBox("[!] #FFFFFFGazete Bırakılıyor, lütfen bekleyiniz.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!] #FFFFFFGazete Bırakılmıştır, devam edebilirsiniz.", 0, 0, 255, true)
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									toggleAllControls(true)
									updategazeteRota()
...
addCommandHandler("gazetelebitir", gazeteBitir)
...
function gazeteAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 510 and vehicleJob == 17 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 17 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Şehir gazeteci Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
