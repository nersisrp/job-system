function makeKey(thePlayer, commandName, keyType, keyID)
	...
	if (logged==1) then
		if(getElementData(thePlayer,"job")==0)then
			if not (keyType) or not (keyID) then
				...
				if not exports.global:hasMoney(thePlayer, 25) then
					...
					if(success)then -- does the player have the key?
						...
						outputChatBox("You have duplicated a ".. keyname .." ".. keyID .." at a cost of TL15.", thePlayer)
						exports.global:meActionLocalSend(thePlayer,"duplicates a ".. keyname ..".")
					else
...
addCommandHandler("anahtar", makeKey, false, false)