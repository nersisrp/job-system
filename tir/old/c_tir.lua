...
local tirRota = {
	...
	{ 2264.232421875, -2439.7099609375, 13.546875, 1, 0 }, -- /tiryukle
...
function updateTirRota()
	...
	for i,v in ipairs(tirRota) do
		if i == tirMarker then
			if v[4] == 2 then
				local theMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 0, 0, 255, getLocalPlayer())
				table.insert(tirCreatedMarkers, { theMarker, 2 })
			elseif v[4] == 1 and v[5] == 0 then
				local dorseMarker1 = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				table.insert(tirCreatedMarkers, { dorseMarker1, 1, 0 })
			elseif v[4] == 1 and v[5] == 1 then
				local dorseMarker2 = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
					...
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							if getVehicleTowedByVehicle(hitVehicle) then
								local hitVehicle = getPedOccupiedVehicle(hitPlayer)
								tirMarker = 0
								outputChatBox("#00FF00[!] #FFFFFFBu turdan TL500 kazandınız.", 0, 0, 255, true)
						...
						elseif marker[2] == true and marker[3] == false then
							if getVehicleTowedByVehicle(hitVehicle) then
								...
								outputChatBox("#00FF00[!] #FFFFFFAracınızın dorsesi indiriliyor, lütfen bekleyiniz.", 0, 0, 255, true)
								setTimer(
									function(thePlayer, hitVehicle, hitMarker)
										...
										detachTrailerFromVehicle(hitVehicle)
										triggerServerEvent("tirParaVer", hitPlayer, hitPlayer)
										outputChatBox("#00FF00[!] #FFFFFFDorseniz indirilmiş ve paranız ödenmiştir (TL400), geri dönebilirsiniz.", 0, 255, 0, true)
	...
	for _, marker in ipairs(tirCreatedMarkers) do
		if marker[2] == 1 and marker[3] == 0 then
			if isElementWithinMarker(getLocalPlayer(), marker[1]) then
				...
				local veh = triggerServerEvent("tir:dorseOlustur", getLocalPlayer())
		...
		elseif marker[2] == 1 and marker[3] == 1 then
			if isElementWithinMarker(getLocalPlayer(), marker[1]) then
				...
				local veh = triggerServerEvent("tir:dorseOlustur2", getLocalPlayer())
				destroyElement(marker[1])
				updateTirRota()
			end
		end
...
addCommandHandler("tiryukle", tirYukle)
...
function tirAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleJob == 6 then
		if thePlayer == getLocalPlayer() and seat ~= 0 then
			...
			setElementFrozen(thePlayer, false)
			outputChatBox("#00FF00[!] #FFFFFFMeslek aracına binemezsiniz.", 255, 0, 0, true)
		elseif thePlayer == getLocalPlayer() and playerJob ~= 10 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("#00FF00[!] #FFFFFFBu araca binmek için Tır Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		end
	end
end
...
function tirBlip()
	blip = createBlip(2273.498046875, -2348.6064453125, 13.546875, 0, 4, 255, 255, 0)  --0 0 1787.1259765625 -1903.591796875 13.394536972046
