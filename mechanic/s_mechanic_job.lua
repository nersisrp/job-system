	...
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			if not getElementData(veh, "Impounded") or getElementData(veh, "Impounded") == 0 then
				exports.nrpSavunma4:changeProtectedElementDataEx(veh, "enginebroke", 0, false)
			...
			exports.global:meActionLocalSend(source, "services the vehicle, patching it up a bit.")
...
function changeTyre( veh, wheelNumber )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			exports.global:meActionLocalSend(source, "replaces the vehicle's tyre.")
		end
	end
end
	...
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			if setVehiclePaintjob( veh, paintjob ) then
				...
				exports.global:meActionLocalSend(source, "repaints the vehicle.")
	...
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			if setVehicleHeadLightColor ( veh, color1, color2, color3) then
				...
				exports.global:meActionLocalSend(source, "replaces the vehicles headlights.")
				exports.nrpSavunma4:changeProtectedElementDataEx(veh, "headlightcolors", {color1, color2, color3}, true)
				exports['savevehicle-system']:saveVehicleMods(veh)
	...
	if (veh) then
		...
		if not item and not exports.global:hasMoney( source, mechcost ) then
			...
			if addVehicleUpgrade( veh, upgrade ) then
				...
				exports.global:meActionLocalSend(source, "added " .. name .. " to the vehicle.")
				exports['savevehicle-system']:saveVehicleMods(veh)
	...
	if veh and stat then
		if stat == 1 then
			...
			--if leader == 1 then
				...
				if not exports.global:takeMoney(source, mechcost) then
					...
					exports.global:meActionLocalSend(source, "begins to place tint on the windows.")
					...
					if query then
						...
						exports.nrpSavunma4:changeProtectedElementDataEx(veh, "tinted", true, true)
						outputChatBox("You have added tint to the vehicle windows.", source)
						exports.global:meActionLocalSend(source, "adds tint to the windows.")
						exports.logs:dbLog(source, 6, {  veh }, "MODDING ADDTINT")
						exports.logs:logMessage("[ADD TINT-LST&R] " .. getPlayerName(source):gsub("_"," ") .. " has added tint to vehicle #" .. vehID .. " - " .. exports.global:getVehicleName(veh) .. ".", 9)
					else
		...
		elseif stat == 2 then
			...
			if not exports.global:takeMoney(source, mechcost) then
				...
				local vehID = getElementData(veh, "dbid")
				exports.global:meActionLocalSend(source, "begins to remove tint from the windows.")
				...
				if query then
					...
					triggerClientEvent("tintWindows", veh)
					exports.nrpSavunma4:changeProtectedElementDataEx(veh, "tinted", false, true)
					outputChatBox("You have cleared the tint from the vehicle windows.", source)
					exports.global:meActionLocalSend(source, "removed tint from the windows.")
					exports.logs:dbLog(source, 6, {  veh }, "MODDING REMOVETINT")
					exports.logs:logMessage("[REMOVED TINT-BTR] " .. getPlayerName(source):gsub("_"," ") .. " has removed tint from vehicle #" .. vehID .. " - " .. exports.global:getVehicleName(veh) .. ".", 9)
				else
