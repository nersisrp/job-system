...
function displayBusJob()
	...
	exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "Çalışmaya başlamak için bir otobüse binin.")
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "Rotaya zaten başladınız.")
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "İşaretleri takip edip duraklarda durun.")
			else
				exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "Rotaya başlayabilmek için bir otobüste olmalısınız.")
			end
		end
	else
		exports.hud:sendBottomNotification(localPlayer, "Notification", "Otobüs şoförü değilsiniz. infoya giderek işe başlayın.")
	end
end
addCommandHandler("busbasla", startBusJob, false, false)
	...
	if thePlayer == getLocalPlayer() then
		...
		if vehicle and bus[getElementModel(vehicle)] then
			if curCpType == 3 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Bus Driver"}, {""}, {"Otobüs durağında bekleyiniz"}, }, false, false, false, 5)
			...
			elseif curCpType == 1 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Bus Driver"}, {""}, {"Yolcuları bekleyin"}, }, false, false, false, 5)
				triggerServerEvent("busAdNextStop", getLocalPlayer(), line, route.points[m_number][5])
			else
				updateBusCheckpoint()
			end
		else
			exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "Otobüs güzergahında gidebilmeniz için bir otobüste olmalısınız.") -- Wrong car type.
		end
	end
end
...
function checkWaitAtStop(thePlayer)
	if thePlayer == getLocalPlayer() then
		if busStopTimer then
			exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "HOOOP! Yolcuları unuttun!")
...
function endOfTheLine()
	if busNextBlip then
		...
		setVehicleLocked(vehicle, false)
		exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "Rotanın sonuna ulaştınız. /busbasla yazarak tekrar baslayabilirsiniz.") --if line is finished
	end
end
			...
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 5)then
						...
						outputChatBox("5TL ödeyecek paranız yok.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("İndi bindi olarak 5TL olarak ödediniz.", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				exports.hud:sendBottomNotification(localPlayer, "Bus Driver", "/busbasla yazarak başlayabilirsiniz.")
			end
		end
	end
end
...
function startEnterBus(thePlayer, seat)
	if seat == 0 and bus[getElementModel(source)] then
		if getVehicleController(source) then -- if someone try to jack the driver stop him
			cancelEvent()
			if thePlayer == getLocalPlayer() then
				exports.hud:sendBottomNotification(localPlayer, "Notification", "Sürücü kapısı kilitli")
			end
		else
