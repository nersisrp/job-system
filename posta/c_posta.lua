-- ROTA --
local postaMarker = 0
local postaCreatedMarkers = {}
local postaRota = {
	--{ 2227.8173828125, -2626.603515625, 13.87215423584, false },
{ 2495.9853515625, -1738.35546875, 13.546875, false }, -- start
...
function postaBasla(cmd)
	if not getElementData(getLocalPlayer(), "postaSoforlugu") then
		local oyuncuArac = getPedOccupiedVehicle(getLocalPlayer())
		local oyuncuAracModel = getElementModel(oyuncuArac)
		local kacakciAracModel = 510
		if oyuncuAracModel == kacakciAracModel then
			setElementData(getLocalPlayer(), "postaSoforlugu", true)
			updatePostaRota()
			addEventHandler("onClientMarkerHit", resourceRoot, postaRotaMarkerHit)
		end
	else
		outputChatBox("[!] #FFFFFFZaten mesleğe başladınız!", 255, 0, 0, true)
	end
end
addCommandHandler("postabasla", postaBasla)

function updatePostaRota()
	postaMarker = postaMarker + 1
	for i,v in ipairs(postaRota) do
		if i == postaMarker then
			if not v[4] == true then
				local rotaMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 0, 0, 255, getLocalPlayer())
				table.insert(postaCreatedMarkers, { rotaMarker, false })
			elseif v[4] == true and v[5] == true then 
				local bitMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				table.insert(postaCreatedMarkers, { bitMarker, true, true })	
			elseif v[4] == true then
				local malMarker = createMarker(v[1], v[2], v[3], "checkpoint", 4, 255, 255, 0, 255, getLocalPlayer())
				table.insert(postaCreatedMarkers, { malMarker, true, false })			
			end
		end
	end
end

function postaRotaMarkerHit(hitPlayer, matchingDimension)
	if hitPlayer == getLocalPlayer() then
		local hitVehicle = getPedOccupiedVehicle(hitPlayer)
		if hitVehicle then
			local hitVehicleModel = getElementModel(hitVehicle)
			if hitVehicleModel == 510 then
				for _, marker in ipairs(postaCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							postaMarker = 0
							triggerServerEvent("postaParaVer", hitPlayer, hitPlayer)
							outputChatBox("[!] #FFFFFFYeni postalar dağıtıma hazırlanıyor. Eğer devam etmek istemiyorsanız, /postabitir yazınız.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!] #FFFFFFYeni postalar dağıtıma hazır.", 0, 255, 0, true)
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitVehicle, true)
							toggleAllControls(false, true, false)
							outputChatBox("[!] #FFFFFFPosta zarfı ayarlanıyor.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!] #FFFFFFZarf posta kutusuna atıldı.", 0, 255, 0, true)
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									toggleAllControls(true)
									updatePostaRota()
...
function postaBitir()
	...
	local postaSoforlugu = getElementData(getLocalPlayer(), "postaSoforlugu")
	if pedVeh then
		if pedVehModel == 510 then
			if postaSoforlugu then
				exports.global:fadeToBlack()
				setElementData(getLocalPlayer(), "postaSoforlugu", false)
				for i,v in ipairs(postaCreatedMarkers) do
					destroyElement(v[1])
				end
				postaCreatedMarkers = {}
				postaMarker = 0
				triggerServerEvent("postaBitir", getLocalPlayer(), getLocalPlayer())
				removeEventHandler("onClientMarkerHit", resourceRoot, postaRotaMarkerHit)
				removeEventHandler("onClientVehicleStartEnter", getRootElement(), postaAntiYabanci)
				setTimer(function() exports.global:fadeFromBlack() end, 2000, 1)
			end
		end
	end
end
addCommandHandler("postabitir", postaBitir)

function postaAntiYabanci(thePlayer, seat, door) 
	local vehicleModel = getElementModel(source)
	local vehicleJob = getElementData(source, "job")
	local playerJob = getElementData(thePlayer, "job")
	
	if vehicleModel == 510 and vehicleJob == 13 then
		if thePlayer == getLocalPlayer() and seat ~= 0 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFMeslek aracına binemezsiniz.", 255, 0, 0, true)
		elseif thePlayer == getLocalPlayer() and playerJob ~= 13 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Posta Taşımacılığı Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		end
	end
end
addEventHandler("onClientVehicleStartEnter", getRootElement(), postaAntiYabanci)

function postaAntiAracTerketme(thePlayer, seat)
...
addEventHandler("onClientVehicleStartExit", getRootElement(), postaAntiAracTerketme)

function postaBlip()
	blip = createBlip(2321.599609375, -2068, 13.546875, 0, 4, 255, 255, 0)  --0 0 1787.1259765625 -1903.591796875 13.394536972046
	exports.hud:sendBottomNotification(localPlayer, "Posta Taşımacılığı Şoförü", "Haritadaki sarı işarete giderek Posta Taşımacılığı şoförlüğü mesleğinize başlayabilirsiniz.")
end