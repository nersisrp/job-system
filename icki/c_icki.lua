local ickiPed = createPed(16, 2571.61328125, -2429, 13.632630348206, 312.83770751953, true)
setElementData(ickiPed, "talk", 1)
setElementData(ickiPed, "name", "Joffrey Yount")
...
function ickiGUI(thePlayer)
	...
	--	if birlikLevel < 3 then
	--		triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] Joffrey Yount diyor ki: " .. levelEksikMsg[math.random(#levelEksikMsg)], 255, 255, 255, 10, {}, true)
	--		outputChatBox("[!] #FFFFFFBu işi yapabilmek için birliğinizin en az 3.seviye olması gerekmektedir.", 255, 0, 0, true)
	--		return
	--	else
	--		triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] Joffrey Yount fısıldar: Hey, elimde bir içki kaçakçılığı işi var. Ne dersin, ha?", 255, 255, 255, 3, {}, true)
	--		ickiKabulGUI(thePlayer)
	--		return
	--	end
	--else
	--	triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] Joffrey Yount diyor ki: Seninle bir işim yok. Derhal toz ol buradan.", 255, 255, 255, 10, {}, true)
	--	return
	--end
...
function ickiKabulGUI(thePlayer)
	...
	local kacakcilikWindow = guiCreateWindow((screenW - 308) / 2, (screenH - 102) / 2, 308, 102, "Nersis Roleplay - İçki Kaçakçılığı", false)
	...
	addEventHandler("onClientGUIClick", ickiKabulBtn, 
		function()
			...
			triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] " .. getPlayerName(thePlayer):gsub("_", " ") .. " diyor ki: " .. kabulEtMsg[math.random(#kabulEtMsg)], 255, 255, 255, 3, {}, true)
			triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] Joffrey Yount diyor ki: Yandaki kamyonlardan birini alarak işe başla, kamyonlar yüklü ve yola çıkmaya hazır. Bol şanslar, ahbap.", 255, 255, 255, 3, {}, true)
			setTimer(function() outputChatBox("[!] #FFFFFFYandaki siyah araçlardan birini alıp, /ickibasla yazarak işe başlayabilirsiniz!", 0, 0, 255, true) end, 500, 1)
			return	
		end
	)
	...
	addEventHandler("onClientGUIClick", ickiIptalBtn, 
		function()
			destroyElement(kacakcilikWindow)
			triggerServerEvent("textNrpLocalSend", getLocalPlayer(), getLocalPlayer(), "[English] " .. getPlayerName(thePlayer):gsub("_", " ") .. " diyor ki: " .. kabulEtmeMsg[math.random(#kabulEtmeMsg)], 255, 255, 255, 3, {}, true)
			return	
		end
	)
end
...
function ickiBasla(cmd)
	...
	if not getElementData(getLocalPlayer(), "ickiKacakciligi") then
		...
		local kacakciAracModel = 413
		
		if (oyuncuBirlikType == 0) or (oyuncuBirlikType == 1) then-- and (oyuncuBirlikLevel >= 3) then
		
			if oyuncuAracModel == kacakciAracModel then

				outputChatBox("[!] #FFFFFFİşiniz başladı iyi yolculuklar!", 255, 0, 0, true)
				setElementData(getLocalPlayer(), "ickiKacakciligi", true)
				updateIckiRota()
				addEventHandler("onClientMarkerHit", resourceRoot, ickiRotaMarkerHit)
			end
		else
			outputChatBox("[!] #FFFFFFBu işe başlamak için illegal bir birlikte olmanız gerekiyor!", 255, 0, 0, true)
		end
	else
