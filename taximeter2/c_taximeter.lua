...
function showTaximeterGui()
	...
	gui_label_totalFare = guiCreateLabel(23,32,108,28,tostring(math.ceil(taxiFare*(taxiDistance/1000))),false,gui_base)
...
function setFare()
	if(source == gui_btn3) then
		...
		gui_fareWin = guiCreateWindow(x,y,width,height,"Set Taximeter Fare",false,nil)
			...
			local lbl2 = guiCreateLabel(20,70,260,20,"TL"..string.format("%.2f", tostring(minFare)).."/km",false,gui_fareWin)
			...
			addEventHandler("onClientGUIScroll", scroll, function()
				if(source == scroll) then
					...
					guiSetText(lbl2, "TL"..tostring(scrollFare).."/km")
