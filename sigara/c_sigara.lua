...
function sigaraBasla(cmd)
	if not getElementData(getLocalPlayer(), "sigaraSoforlugu") then
		...
		if oyuncuAracModel == kacakciAracModel then
			setElementData(getLocalPlayer(), "sigaraSoforlugu", true)
			updatesigaraRota()
				...
				for _, marker in ipairs(sigaraCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!] #FFFFFFYeniden sürmek  için hazırlanıyoruz, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /sigarabitir yazınız.", 0, 0, 255, true)
							setTimer(
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitVehicle, true)
							toggleAllControls(false, true, false)
							outputChatBox("[!] #FFFFFFSigaralar Bırakılıyor, lütfen bekleyiniz.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!] #FFFFFFSigaralar Bırakılmıştır, devam edebilirsiniz.", 0, 0, 255, true)
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									toggleAllControls(true)
									updatesigaraRota()
...
function sigaraBitir()
	...
	local sigaraSoforlugu = getElementData(getLocalPlayer(), "sigaraSoforlugu")
	if pedVeh then
		if pedVehModel == 459 then
			if sigaraSoforlugu then
				exports.global:fadeToBlack()
				setElementData(getLocalPlayer(), "sigaraSoforlugu", false)
...
addCommandHandler("sigaralebitir", sigaraBitir)
...
function sigaraAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 459 and vehicleJob == 17 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 17 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Şehir sigaracı Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
