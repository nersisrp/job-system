...
function ickiBasla(cmd)
	if not getElementData(getLocalPlayer(), "ickiSoforlugu") then
		...
		if oyuncuAracModel == kacakciAracModel then
			setElementData(getLocalPlayer(), "ickiSoforlugu", true)
			updateickiRota()
				...
				for _, marker in ipairs(ickiCreatedMarkers) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							outputChatBox("[!] #FFFFFFYeniden sürmek  için hazırlanıyoruz, lütfen bekleyiniz. Eğer devam etmek istemiyorsanız, /ickibitir yazınız.", 0, 0, 255, true)
							setTimer(
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitVehicle, true)
							toggleAllControls(false, true, false)
							outputChatBox("[!] #FFFFFFİçkiler Bırakılıyor, lütfen bekleyiniz.", 0, 0, 255, true)
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									outputChatBox("[!] #FFFFFFİçkiler Bırakılmıştır, devam edebilirsiniz.", 0, 0, 255, true)
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									toggleAllControls(true)
									updateickiRota()
...
function ickiBitir()
	...
	local ickiSoforlugu = getElementData(getLocalPlayer(), "ickiSoforlugu")
	if pedVeh then
		if pedVehModel == 413 then
			if ickiSoforlugu then
				exports.global:fadeToBlack()
				setElementData(getLocalPlayer(), "ickiSoforlugu", false)
...
addCommandHandler("ickilebitir", ickiBitir)
...
function ickiAntiYabanci(thePlayer, seat, door) 
	...
	if vehicleModel == 413 and vehicleJob == 17 then
		...
		elseif thePlayer == getLocalPlayer() and playerJob ~= 17 then
			setElementFrozen(thePlayer, true)
			setElementFrozen(thePlayer, false)
			outputChatBox("[!] #FFFFFFBu araca binmek için Şehir ickicu Şoförlüğü mesleğinde olmanız gerekmektedir.", 255, 0, 0, true)
		cancelEvent()
		end
	end
end
